...
| `infrastructure/deploy/DEPLOYMENT_RUNBOOK.md` (histórico — fusionado en [`docs/deployment/homelab-deployment.md`](../deployment/homelab-deployment.md) el 2026-07-15) | *(eliminado, ver enlace)* | Preexistente (issue #45). Dos referencias a la ruta antigua `docker/docker-compose.prod.yml` actualizadas a la nueva ubicación. |
