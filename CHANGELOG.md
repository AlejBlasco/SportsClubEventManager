...
- `infrastructure/docker-compose/docker-compose.prod.yml`: two comments still pointed at the now-deleted `DEPLOYMENT_RUNBOOK.md` — redirected to `docs/deployment/homelab-deployment.md`
- `infrastructure/docker-compose/README.md`: same misattribution already fixed elsewhere this session — the WSL2 mount-propagation troubleshooting entry blamed `cadvisor`, but it's `node-exporter` (`rslave` propagation) that actually fails; `cadvisor`'s plain `:ro` mount is unaffected. Also removed a stale "pending manual step" note about updating the Portainer stack's compose file path after the `docker/` → `infrastructure/docker-compose/` move — verified already resolved (9 successful `deployed/homelab/*` tags since, which wouldn't exist if Portainer were still pointing at the old path)
- Removed `infrastructure/n8n/README.md` — a one-time manual runbook checklist for the homelab owner (issue #37), not living documentation; all of its steps are already completed in production. Its content is now historical, referenced (not duplicated) from `docs/technical/issue-37-workflows-n8n-notificaciones-eventos.md`. While reviewing it, verified against the real production `api` container and its live Prometheus metrics that the checklist was stale on 2 of its 3 items marked pending: webhook URLs in prod `.env` were actually already set (`Notifications__N8n__Enabled=true`), and end-to-end verification had already happened for `registration-confirmed` (one real `result="success"`) — still genuinely pending for `event-updated`/`event-cancelled`/`event-reminder` (never triggered by the app in production) and for error-monitoring verification (no `result="failure"` series exists), noted in the design doc rather than tracked as an open checklist going forward
